...
class Controlador:
    ...
    def sonido_laser(self):
        ...
        env = Fader(fadein=0.01, fadeout=duracion - 0.01, dur=duracion, mul=0.5).play()

        curva_freq = Linseg(
            list=[(0, freq_inicial), (duracion, freq_final)]
        ).play()

        # Antes: self.laser_sine = Sine(...).out()
        # Ahora: sumamos a la salida principal
        fx = Sine(freq=curva_freq, mul=env)

        fx.out()  #Escuchar en vivoS
        self.sint.salida += fx  # Mezcla en la salida principal
        self.laser_sine = fx
