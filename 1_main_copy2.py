...
class Controlador:
    ...
    def sonido_szoom(self):
        ...
        freq_inicial = np.random.uniform(150, 250)
        freq_pico = np.random.uniform(600, 800)
        freq_final = np.random.uniform(120, 200)
        duracion = 0.8

        env = Fader(fadein=0.05, fadeout=0.2, dur=duracion, mul=3).play()

        curva_freq = Linseg(
            list=[
                (0, freq_inicial),
                (duracion * 0.4, freq_pico),
                (duracion, freq_final)
            ]
        ).play()

        seno = Sine(freq=curva_freq, mul=env*0.7)
        ruido = ButLP(Noise(mul=env*0.3), freq=1000)
